...
## Provider preprocessing recovered exactly (2026-09-09)

`datasets/dmir/scenario_race.py` rebuilds classification-layout windows from the
raw H5 sessions and checks them against the test pickle (6,063 windows of the
two test drivers, max deviation 1.4e-7). What the check established:

- every channel is an affine map `pickle = a * raw + b` of one raw signal;
  the map is identical for both drivers (agreement 2e-10), i.e. one scaler
  fitted once on the training split;
- the two indicator channels (28, 29) are **unscaled** 0/1;
- `egoLaneWidth` (7) maps to exactly 0;
- **car1 (16-21) = the object nearest in |LongPosition| among object slots
  1-10** of the H5 `objects` group (slot 0 is the ego itself); ahead or behind,
  any lane;
- **car2 (22-27) = object slot 11**, the scenario's dedicated "car on the
  left"; `car2Present` (30) = that slot's ID != -1; an absent car is all zeros
  (raw), i.e. the constant `b` after scaling;
- `rightLaneLine`/`leftLaneLine` (14, 15) are the two ego lane-line distances
  `Dy[0]`/`Dy[1]` (they sum to 3.75 m, hence the exact anti-correlation).

The per-channel `a`, `b` are stored in `datasets/dmir/data/race/norm_cls.json`
(gitignored, regenerated by `scenario_race.py build`).
