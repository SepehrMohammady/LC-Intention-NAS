...
The per-layer breakdown explains why. Both our model and the reference spend
...
($\sim$95\% of the model), whereas the searched models pool first and flatten
only 308 (or 98) features.

RAM behaves differently and is worth stating plainly. Both searched classifiers
measure $\sim$9.2~KB regardless of a $9\times$ parameter gap, because the
$50\times31$ float32 input tensor alone occupies 6.2~KB and no architecture can
go below it. The regression model instead measures 20.8~KB: its widest branch
emits a $25\times116$ tensor of 11.6~KB, so the peak working set is that plus
the input. Peak RAM is therefore $\max(\text{input tensor},\ \text{widest
layer's in\,+\,out})$---set by layer width rather than depth or parameter
count---which is why the RAM advantage ($\sim$4$\times$) is far smaller than the
flash advantage, and why an int8 input, not a smaller network, is the lever that
