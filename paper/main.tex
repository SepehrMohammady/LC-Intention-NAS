...
The search independently arrives at three microcontroller-friendly choices:
depthwise-separable convolutions, a residual connection, and aggressive early
downsampling. The classification model reduces the sequence $50 \rightarrow 25
...
RAM behaves differently and is worth stating plainly: both searched models
measure $\sim$9.2~KB regardless of a $9\times$ parameter gap, because the
$50\times31$ float32 input tensor alone occupies 6.2~KB. On this task RAM is
input-bound rather than model-bound, which is why the RAM advantage
($\sim$4$\times$) is far smaller than the flash advantage, and why an int8
input---not a smaller network---is the lever that would reduce it.
