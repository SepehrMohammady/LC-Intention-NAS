...
are our own test-set evaluation of the saved models. \emph{These are
preliminary}: each search stopped before its target budget and will be re-run
to completion. \todo{replace with final full-search numbers; add peak-RAM,
MACs, and measured on-device latency columns from ST Edge AI.}
...
\midrule
Ours (searched, best)                & 216 & 91.6 & 0.290 & 0.320 \\
Ours (searched, compact)             & 21  & 91.5 & 0.294 & 0.321 \\
Ours, no turn-signal (ablation)      & 5.2 & 90.1 & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

The searched models improve on the published SOTA MAE (0.290 vs 0.298 for LCR)
at a wide range of footprints, down to a few tens of kilobytes. Removing the
turn-signal channels (the leaky feature) drops three-class accuracy by under
one point (91.6\% to 90.8\%), and a 5.2~KB model without them still reaches
90.1\%---evidence that the model anticipates from vehicle dynamics and
...
\cite{lcintention2026}, which deployed FP32 models only.
\todo{toolchain version, int8 quantization accuracy drop, flash / RAM /
latency measured on device, comparison with the baseline's Table III.}
