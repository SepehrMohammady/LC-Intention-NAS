...
\begin{tabular}{lccc}
\toprule
 & Reference CNN & Ours & Gain \\
\midrule
Accuracy (\%)   & 91.7  & \textbf{92.1} & --- \\
Latency (ms)    & 33.52 & \textbf{3.63} & $9.2\times$ \\
Flash (KB)      & 1729  & \textbf{335}  & $5.2\times$ \\
RAM (KB)        & 38.3  & \textbf{9.2}  & $4.1\times$ \\
MACC (M)        & 1.97  & \textbf{0.16} & $12.4\times$ \\
\bottomrule
\end{tabular}
\end{table}

The searched model is $9.2\times$ faster and $5.2\times$ smaller than the
reference CNN while being more accurate. The per-layer breakdown explains why:
both models spend most of their flash in the fully-connected head, but the
reference flattens the whole $50\times128$ feature map (its single
$64\times6400$ layer is 1.64~MB, $\sim$95\% of the model), whereas the searched
model pools first, shrinking the same head to 280~KB.
